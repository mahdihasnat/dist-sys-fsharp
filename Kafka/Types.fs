...
module Codecs =
    let propMapOfLogKey (codec: Codec<'Encoding, 'a>) =
        (Ok << Map.ofSeq << Seq.map (Tuple2.mapItem1 LogKey) << PropertyList.ToSeq
         <-> (Map.toArray >> Array.map (Tuple2.mapItem1 (fun (LogKey x) -> x)) >> PropertyList))
...
type OutputMessageBody =
    ...
    static member ToJson (x: OutputMessageBody) =
        match x with
        | SendAck (inReplyTo, offset) -> jobj [ "type" .= "send_ok"; "in_reply_to" .= inReplyTo; "offset" .= offset ]
        ...
        | CommitOffsetsAck inReplyTo -> jobj [ "type" .= "commit_offsets_ok"; "in_reply_to" .= inReplyTo ]
