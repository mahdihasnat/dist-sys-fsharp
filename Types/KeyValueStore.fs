...
type KVRequestMessageBody<'Value> =
    ...
    static member inline ToJson (x: KVRequestMessageBody<_>) =
        match x with
        | KVRequestMessageBody.Read (messageId, key) -> jobj [ "type" .= "read"; "msg_id" .= messageId; "key" .= key ]
        | KVRequestMessageBody.Write (messageId, key, ``value``) ->
            jobj [ "type" .= "write"; "msg_id" .= messageId; "key" .= key; "value" .= ``value`` ]
    ...
    static member inline OfJson json =
        ...
        | JObject o ->
            monad {
                ...
                | s when s = "write_ok" -> return KVResponseMessageBody.WriteOk inReplyTo
                | s when s = "cas_ok" -> return KVResponseMessageBody.CompareAndSwapOk inReplyTo
                | s when s = "error" ->
                    ...
                    | 22 -> return KVResponseMessageBody.ErrorPreconditionFailed inReplyTo
                    | _ -> return! Error <| DecodeError.Uncategorized $"Error code is not 20 or 22, code = {code}"
                | _ -> return! Error <| DecodeError.Uncategorized $"Message type is not supported: {msgType}"
            }
