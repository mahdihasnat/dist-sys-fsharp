...
module Codec =
    let inline isomorph
        (enc: 'U -> 'T)
        (dec: 'T -> 'U)
        (codec1: Codec<'a, 'a, 'T, 'T>)
        : Codec<'a, 'a, 'U, 'U> when 'a :> IEncoding and 'a: (new: unit -> 'a) =
...
type Variant<'Variant1, 'Variant2, ^Encoding
    ...
    static member inline get_Codec () =
        ...
        let decoder1 = (Codec.decode codec1) >> (Result.map Variant1Of2)
        let decoder2 = (Codec.decode codec2) >> (Result.map Variant2Of2)
...
type Variant<'Variant1, 'Variant2, 'Variant3, ^Encoding
    ...
    static member inline get_Codec () =
        ...
        let decoder1 = (Codec.decode codec1) >> (Result.map Variant1Of3)
        let decoder2 = (Codec.decode codec2) >> (Result.map Variant2Of3)
        let decoder3 = (Codec.decode codec3) >> (Result.map Variant3Of3)
