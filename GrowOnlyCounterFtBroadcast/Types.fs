...
with
    static member get_Codec () =
        codec {
            let! (msgType) = jreqAlways "type" (function | Add _ -> "add" | Read _ -> "read" | Gossip _ -> "gossip" | GossipAck _ -> "gossip_ok")
            and! (messageId : Option<MessageId>) = jopt "msg_id" (function | Add (messageId, _) | Read messageId |  Gossip (messageId, _) -> Some messageId | GossipAck _ -> None)
            and! delta = jopt  "delta" (function | Add (_, delta) -> Some delta | Read _ | Gossip _ | GossipAck _ -> None)
            and! transactions = jopt "transactions" (function | Gossip (_, messages) -> Some messages | Add _ | Read _ | GossipAck _-> None)
