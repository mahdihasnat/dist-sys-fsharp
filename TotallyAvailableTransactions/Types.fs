        ...
        | JArray arr ->
            monad {
                ...
                | s when s = "r" ->
                    ...
                    | None -> return Read key
                    | Some _unit -> return! (Error (DecodeError.Uncategorized $"Unexpected value in read operation: {arr.Item 2}"))
                | s when s = "w" ->
                    let! (key: Key) = ofJson (arr.Item 1)
                    let! (value: Value) = ofJson (arr.Item 2)
                    return Write (key, value)
                | x -> return failwithf $"Unknown operation type: {x}"
            }
...
type TransactionOperationOutput =
    ...
    static member ToJson (x: TransactionOperationOutput) =
        match x with
        | Read (key, value) -> JArray [ toEncoding "r"; toEncoding key; toEncoding value ]
        | Write (key, value) -> JArray [ toEncoding "w"; toEncoding key; toEncoding value ]
...
type OutputMessageBody =
    ...
    static member ToJson (x: OutputMessageBody) =
        match x with
        | TxnAck (inReplyTo, ops) -> jobj [ "type" .= "txn_ok"; "in_reply_to" .= inReplyTo; "txn" .= ops ]
        | KVRequest kvRequestMessageBody -> KVRequestMessageBody<Value>.ToJson kvRequestMessageBody
...
type Node =
    ...
    member this.RegisterReadOkHandler (messageId: MessageId) (handler: Node -> Value -> TransitionResult) =
        { this with
            OnKVReadOkHandlers = this.OnKVReadOkHandlers.Add (messageId, handler)
        }

    member this.RegisterWriteOkHandler (messageId: MessageId) (handler: Node -> TransitionResult) =
        { this with
            OnKVWriteOkHandlers = this.OnKVWriteOkHandlers.Add (messageId, handler)
        }

    member this.RegisterErrorKeyDoesNotExistHandler (messageId: MessageId) (handler: Node -> TransitionResult) =
        { this with
            OnKVErrorKeyDoesNotExistHandlers = this.OnKVErrorKeyDoesNotExistHandlers.Add (messageId, handler)
        }

    member this.RegisterCompareAndSwapOkHandler (messageId: MessageId) (handler: Node -> TransitionResult) =
        { this with
            OnKVCompareAndSwapOkHandlers = this.OnKVCompareAndSwapOkHandlers.Add (messageId, handler)
        }

    member this.RegisterErrorPreconditionFailedHandler (messageId: MessageId) (handler: Node -> TransitionResult) =
        { this with
            OnKVErrorPreconditionFailedHandlers = this.OnKVErrorPreconditionFailedHandlers.Add (messageId, handler)
        }
