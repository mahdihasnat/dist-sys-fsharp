...
let rec getAndWriteTransformedValueAndVersion node (mapper: Value * Version -> Value * Version) (f: (Node * (Value * Version)) -> TransitionResult) : TransitionResult =
    transition {
        ...
        let node =
            {
                node with
                    ...
                    OnSeqKVCompareAndSwapPreconditionFailedHandlers = node.OnSeqKVCompareAndSwapPreconditionFailedHandlers.Add(updateMessageId, fun node -> getAndWriteTransformedValueAndVersion node mapper f)
            }
        return node
    }
