...
type BroadCastMessageBody =
    ...
    static member get_Codec () =
        BroadCastMessageBody.get_ObjCodec () |> ofObjCodec
...
type ReadMessageBody =
    ...
    static member get_Codec () =
        ReadMessageBody.get_ObjCodec () |> ofObjCodec
...
type TopologyMessageBody =
    ...
    static member get_Codec () =
        TopologyMessageBody.get_ObjCodec () |> ofObjCodec
...
module Codecs =
    let propMapOfNodeId (codec: Codec<'Encoding, 'a>) =
        (Ok << Map.ofSeq << Seq.map (Tuple2.mapItem1 NodeId) << PropertyList.ToSeq
         <-> (Map.toArray >> Array.map (Tuple2.mapItem1 (fun (NodeId x) -> x)) >> PropertyList))
        >.> Codecs.propList codec
        ...
        codec {
            ...
            and! (messageId: Option<MessageId>) =
                jopt "msg_id" (function
                    | BroadCast (messageId, _)
                    | Read messageId
                    | Topology (messageId, _)
                    | Gossip (messageId, _) -> Some messageId
                    | GossipAck _ -> None)

            and! (message: Option<int>) =
                jopt "message" (function
                    | BroadCast (_, message) -> Some message
                    | Gossip _
                    | Read _
                    | Topology _
                    | GossipAck _ -> None)
            ...
            and! topology =
                joptWith (Codecs.option (Codecs.propMapOfNodeId defaultCodec<_, Set<NodeId>>)) "topology" (function
                    | Topology (_, topology) -> Some (topology)
                    | _ -> None)

            and! messages =
                jopt "messages" (function
            ...
            match msgType with
            | s when s = "broadcast" -> return BroadCast (messageId |> Option.get, message |> Option.get)
            | s when s = "read" -> return Read (messageId |> Option.get)
            | s when s = "topology" -> return Topology (messageId |> Option.get, topology |> Option.get)
            | s when s = "gossip" -> return Gossip (messageId |> Option.get, messages |> Option.get)
    ...
    static member get_Codec () =
        codec {
            let! msgType =
                jreqAlways "type" (function
                    | ReadAck _ -> "read_ok"
                    | BroadCastAck _ -> "broadcast_ok"
                    | TopologyAck _ -> "topology_ok"
                    | GossipAck _ -> "gossip_ok"
                    | Gossip _ -> "gossip")
            ...
            and! inReplyTo =
                jopt "in_reply_to" (function
                    | ReadAck (inReplyTo, _)
                    | BroadCastAck inReplyTo
                    | TopologyAck inReplyTo
                    | GossipAck inReplyTo -> Some inReplyTo
                    | Gossip _ -> None)
            ...
            match msgType with
            | s when s = "read_ok" -> return ReadAck (inReplyTo |> Option.get, messages |> Option.get)
            | s when s = "broadcast_ok" -> return BroadCastAck (inReplyTo |> Option.get)
            | s when s = "topology_ok" -> return TopologyAck (inReplyTo |> Option.get)
            | s when s = "gossip_ok" -> return GossipAck (inReplyTo |> Option.get)
            | s when s = "gossip" -> return Gossip (messageId |> Option.get, gossipMessages |> Option.get)
